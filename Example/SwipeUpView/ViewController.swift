...
class ViewController: UIViewController {
    ...
    lazy var bottomView : BottomView = {
        return BottomView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 0))//.zero)
    }()
    
    lazy var swipeUpView : SwipeUpView = {
        let mg = SwipeUpView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 0), mainView: self.view)
        mg.delegate = self
        mg.datasource = self
...
extension ViewController : SwipeUpViewDatasource ,SwipeUpViewDelegate {
    ...
    func animationUpSpeed(_ swipeUpView: SwipeUpView) -> TimeInterval {
        return 0.5
    }
    
    func animationDownSpeed(_ swipeUpView: SwipeUpView) -> TimeInterval {
        return 0.2
    }
